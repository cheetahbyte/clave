...
func (s *Service) SharedSecret(clientPubKeyBytes []byte) ([]byte, error) {
	curve := ecdh.X25519()
...
func (s *Service) Encrypt(plaintext, aesKey []byte) ([]byte, error) {
...
func (s *Service) Decrypt(data, aesKey []byte) ([]byte, error) {
	block, err := aes.NewCipher(aesKey)
	if err != nil {
		return nil, err
	}
