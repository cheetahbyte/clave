...
const chartConfig = {
  activations: { label: "Activations", color: "var(--chart-1)" },
  validations: { label: "Validations", color: "var(--chart-3)" },
} satisfies ChartConfig;

type Metric = keyof typeof chartConfig;

function UsageChart() {
  const [metric, setMetric] = React.useState<Metric>("activations");

  // Random demo data — 30 days of usage.
  const data = React.useMemo(() => {
    ...
    return Array.from({ length: days }, (_, i) => {
      ...
      return {
        ...
        activations: Math.floor(20 + Math.random() * 80),
        validations: Math.floor(120 + Math.random() * 380),
      };
    });
  }, []);
    ...
    <Card>
      <CardHeader className="flex flex-col gap-2 space-y-0 sm:flex-row sm:items-center sm:justify-between">
        <div className="space-y-1">
          ...
          <CardDescription>
            Daily {chartConfig[metric].label.toLowerCase()} over the last 30 days
          </CardDescription>
        </div>
        <Select value={metric} onValueChange={(v) => setMetric(v as Metric)}>
          <SelectTrigger className="w-40" aria-label="Select metric">
            <SelectValue />
          </SelectTrigger>
          <SelectContent>
