...
export async function requestSelfServiceLink(
  email: string,
  orgSlug: string,
): Promise<{ ok: boolean }> {
  return selfServiceFetch<{ ok: boolean }>(
    "/api/v1/self-service/auth/request-token",
    {
      method: "POST",
...
export async function validateSelfServiceToken(
  token: string,
  orgSlug: string,
): Promise<{ ok: boolean; expiresAt: number }> {
  return selfServiceFetch<{ ok: boolean; expiresAt: number }>(
    "/api/v1/self-service/auth/validate",
    {
      method: "POST",
...
export async function checkSelfServiceSession(): Promise<boolean> {
  try {
    const res = await fetch("/api/v1/self-service/session", {
      credentials: "include",
    });
...
export async function listSelfServiceDevices(
  licenseId: string,
): Promise<Device[]> {
  return selfServiceFetch<Device[]>(
    `/api/v1/self-service/licenses/${licenseId}/devices`,
  );
}

export async function removeSelfServiceDevice(
  licenseId: string,
  deviceId: string,
): Promise<{ ok: boolean }> {
  return selfServiceFetch<{ ok: boolean }>(
...
export async function revokeSelfServiceLicense(
  licenseId: string,
): Promise<{ ok: boolean; newLicenseId: string }> {
